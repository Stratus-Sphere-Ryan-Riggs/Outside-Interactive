 ...
 function (serverWidget, record, error, search, format, runtime, render, email, task) {
     ...
     function onRequest(context) {
         ...
         try {
             ...
             sublistCustomers.addField({
                 id: 'emailoverride',
                 label: 'Override Email Recipients',
                 type: serverWidget.FieldType.TEXT
             }).updateDisplayType({
                 displayType: serverWidget.FieldDisplayType.ENTRY
             });
                 ...
                 if (!empty(optionFlag)) {
                     ...
                     if (optionFlag == 'Statement') {
                         ...
                         for (var i = 0; i < lineCount; i++) {
                             ...
                             var overrideEmails = contextRequest.getSublistValue({ group: 'listcustlevel', name: 'emailoverride', line: i });

                             if (lineSelected == 'T') {
                                 var dunningLevel = "Statement";
                                 log.audit("a", "override: " + overrideEmails);
                                 var custObj = {
