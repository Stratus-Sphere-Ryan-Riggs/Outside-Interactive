 ...
 */
 define(['N/record', 'N/search', 'N/runtime', 'N/format', 'N/runtime', 'N/render', 'N/email', 'N/xml', 'N/file', '../common/SS_Constants'],
 function (record, search, runtime, format, runtime, render, email, xml, file, SS_Constants) {
     ...
     function getInputData() {
         ...
         try {
            ...
            let updateValues = {};
            updateValues[TASK.Fields.STATUS] = SS_Constants.CustomLists.DataTableTaskStatus.IN_PROGRESS;
            record.submitFields({
                type: TASK.Id,
                id: taskRecordId,
                values: updateValues
            ...
            let taskRecordLookup = search.lookupFields({
                type: TASK.Id,
                id: taskRecordId,
                columns: [
                    TASK.Fields.DATA
            ...
            log.debug({ title: `${logTitle} logTitle`, details: JSON.stringify(taskRecordLookup) });

            let taskRecordData = taskRecordLookup[TASK.Fields.DATA];
            if (!taskRecordData) { return; }

            let taskRecordDataJSON = JSON.parse(taskRecordData);
            let list = taskRecordDataJSON.list;
            list = list.map(d => {
            ...
            log.debug({ title: `${logTitle} list`, details: JSON.stringify(list) });
